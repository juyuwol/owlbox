...
{
  "id": "8ot58g000",
  "sent": "1991-07-31T00:00:00.000+00:00",
  "message": "생일 축하한다 해리 🎂",
  "ip": "192.0.2.0"
}
```

- `id`: 쪽지의 고유 식별자. 수신 시각에 대한 ‘epoch 시간(UTC 시간대에서의 1970년 1월 1일 정각)으로부터 경과한 밀리초’(JavaScript Date 객체의 단위)의 36진수 표기 뒤에 문자 `0`을 더한 형식입니다.
   ...
   Date.parse("1991-07-31T00:00:00.000+00:00").toString(36) + '0'
   ```
...
각 쪽지의 카드 이미지(쪽지/답장 URL을 트윗 등으로 공유할 때 뜨는 대형 섬네일)에 대한 텍스트, 색상, 여백 등의 서식은 리포지토리의 `layouts/card.js`에 위치한 모듈 파일에서 구성됩니다. 이 모듈은 문자열 배열 `fonts`와 `style` 객체 각각을 `export` 구문을 통해 내보내야 합니다.
...
- 색상
   ...
   아래의 속성들은 RGB 값을 표현하는 8비트 정수 3개의 배열이어야 합니다. 예를 들어, `[0x00, 0xff, 0x00]`은 HEX 코드 `#00ff00`의 형광초록색이 됩니다.
...
- 레이아웃
   ...
   - `horizontalFrameThickness`: 프레임 가로줄의 두께
   - `verticalFrameThickness`: 프레임 세로줄의 두께
   - `frameLeft`: 전체 이미지 왼쪽에서부터 프레임 왼쪽까지의 거리
   - `frameTop`: 전체 이미지 상단에서부터 프레임 상단까지의 거리
   - `contentLeft`: 전체 이미지 왼쪽에서부터 텍스트 영역 왼쪽까지의 거리
   - `minContentTop`: 전체 이미지 상단에서부터 텍스트 영역 상단까지의 최소 거리

## 레이아웃 커스터마이징

각 쪽지/답장 페이지 및 목록의 레이아웃 및 그 밖의 페이지는 리포지토리의 `layouts/index.js`에 위치한 모듈 파일에서 구성됩니다. 이 모듈에서 `renderList` 함수, `renderPost` 함수, `hooks` 객체, `pages` 객체 각각을 `export` 구문을 통해 내보내야 합니다. 상세는 [layouts.d.ts](layouts.d.ts) 파일에 있습니다.
...
```
node src/build.js --output <output directory> [--production | --proxy]
```

`--production` 및 `--proxy` 플래그를 설정하지 않으면 개발 환경 취급입니다. `<output directory>`에 사이트 전체의 페이지와 파일이 생성되기에, 이곳을 루트로 삼아 HTTP 서버를 실행하면 됩니다.
