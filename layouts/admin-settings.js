...
data-schema='${JSON.stringify({
  ...
  <ul class="settings-list">
    <li class="settings-item"><label class="label-checkbox">\
<input name="activated" type="checkbox"${(site.activated === true) ? ' \
checked=""' : ''} aria-describedby="activated-description"> \
쪽지 받기</label><br>
...
활성 상태로 설정해도 무시됩니다.'}</span></li>
    <li class="settings-item"><label class="label-checkbox">\
<input name="notify" type="checkbox"${(site.notify === true) ? ' \
checked=""' : ''}${sendable ? '' : ' aria-describedby="notify-description"'}> \
...
이메일 주소를 설정하지 않으면, 활성 상태로 설정해도 무시됩니다.</span>`}</li>
    <li class="settings-item"><label class="settings-field">\
...
type="number" value="${site.maxLength}" required="" min="1" max="1000">\
</label></li>
    <li class="settings-item"><label class="settings-field">\
목록의 최대 표시 수: <input class="settings-input" name="perPage" \
type="number" value="${site.perPage}" required="" min="1" max="32767">\
</label></li>
    <li class="settings-item"><label class="settings-field">\
...
모든 페이지 하단(푸터)에 위치한 링크의 URL이 됩니다.</span></li>
    <li class="settings-item"><label class="settings-field-textarea">\
...
메인 페이지 ‘이곳에 대해’ 소제목 아래의 상세 소개문입니다. (HTML)</span></li>
    <li class="settings-item"><label class="label-checkbox">\
<input name="showLogin" type="checkbox"${(site.showLogin === true) ? ' \
checked=""' : ''} aria-describedby="show-login-description"> \
로그인 메뉴 보이기</label><br>
...
비활성화 시, 그 자리에 다크/라이트 테마 전환 토글이 들어갑니다.</span></li>
    <li>선호 이메일 발송자:<br>
    <label class="label-checkbox"><input name="preferredSender" type="radio" \
value="google"${(preferredSender === 'google') ? ' \
checked=""' : ''} aria-describedby="preferred-sender-description"> \
Google Apps Script</label><br>
    <label class="label-checkbox"><input name="preferredSender" type="radio" \
value="resend"${(preferredSender === 'resend') ? ' \
checked=""' : ''} aria-describedby="preferred-sender-description"> \
Resend</label><br>
...
우선하여 사용할 발송자를 선택합니다.</span></li>
  ...
  <p class="settings-submit">\
<button type="submit" disabled="">변경</button></p>
</form>`));
};
